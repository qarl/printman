...
int run_usd(const std::string& usd_in, const std::string& out, const std::string& aovname,
    ...
    // Slice each mesh. A Catmull-Clark surface goes through the band loop at a uniform level (its
    // cage is uniform). A displaced quad polygon mesh takes the ADAPTIVE band loop (bound-and-split:
    // each face dices to its own edge scale to the world tolerance -- bead width for the slice, ~1px
    // for the render). A displaced non-quad polygon mesh falls back to uniform bilinear dicing. A
    // flat, undisplaced mesh (a colour-only robot) is materialized -- dicing adds nothing.
    const double slice_tol = line_width;
    ...
    for (size_t i = 0; i < meshes.size(); ++i) {
        ...
        if (poly) {                                         // adaptive dice + displace (quads direct, tri/n-gon center-split)
            ...
            if (slice_level == render_level && slice_tol == render_tol) {
                nbands_total += nb;
                L = amplify_usd_adaptive(meshes[i], shaders[i], slice_level, slice_tol, zs, nb, renderhook, /*do_slice*/ true);
            } else {
                nbands_total += 2 * nb;
            ...
            size_t corners = 0; for (int c : meshes[i].counts) corners += (size_t)c;
            unique_tri += corners - 2 * meshes[i].counts.size();
...
int run_usd_gate(const std::string& usd_in, const std::string& shaderdir, const std::string& osl_disp,
    ...
    const double gate_tol = me > 0 ? me / double(1 << std::max(level, 1)) : 0.4;
    auto A = adaptive ? amplify_usd_adaptive(uc, shaders, level, gate_tol, zs, 1)
                      : amplify_usd_banded(uc, shaders, level, zs, 1);
    auto B = adaptive ? amplify_usd_adaptive(uc, shaders, level, gate_tol, zs, bands)
                      : amplify_usd_banded(uc, shaders, level, zs, bands);
    ...
    std::printf("usd-gate: band-count=1 vs band-count=%d over %zu layers @ %dx%d px (subdiv %d, %s)\n",
                bands, zs.size(), win.W, win.H, level, adaptive ? "adaptive" : "uniform");
